...
Este documento reúne as evidências capturadas no Swagger e no terminal para demonstrar o funcionamento da API BrightSpot. As imagens e responses estão na pasta [`docs/evidencias`](evidencias).
...
- O print [`terminal_credenciais_bancodedados.png`](evidencias/terminal_credenciais_bancodedados.png) foi redigido para ocultar a senha do Oracle e evitar exposição de credenciais reais.
- O arquivo [`response_get_api-devices-deviceid-alerts.txt`](evidencias/response_get_api-devices-deviceid-alerts.txt) contém a lista de alertas duplicada no mesmo arquivo. A evidência continua válida, mas uma nova captura com apenas uma resposta deixaria a leitura mais limpa.
- A response de [`POST /api/devices/{deviceId}/alerts`](evidencias/response_post_api-devices-deviceid-alerts.txt) mostra `severity: 0`. Em uma demonstração padronizada, o ideal é registrar alertas com `severity` entre `1` e `3`, conforme o enum da API: `1 = Info`, `2 = Warning`, `3 = Critical`.
- Algumas evidências usam nomes de sensores livres, como `"SENSOR DE TEMPERATURA"`, mesmo quando o `sensorType` selecionado representa outro tipo. Isso não quebra a API, mas nomes alinhados ao tipo escolhido deixam a demonstração mais clara.

## Evidências de ambiente

### Connection string Oracle configurada

![Connection string Oracle configurada](evidencias/terminal_credenciais_bancodedados.png)
...
![API executando com dotnet run](evidencias/terminal_dotnetrun.png)
...
![Swagger funcionando](evidencias/url_swagger_funcionando.png)
...
![POST /api/devices](evidencias/print_post_api-devices.png)
...
![GET /api/devices](evidencias/print_get_api-devices.png)
...
![GET /api/devices/{id}](evidencias/print_get_api-devices-id.png)
...
![PATCH /api/devices/{id}/status](evidencias/print_patch_api-devices-id-status.png)
...
![PATCH /api/devices/{id}/configuration](evidencias/print_patch_api-devices-id-configuration.png)
...
![POST /api/devices/{id}/sensors](evidencias/print_post_api-devices-id-sensors.png)
...
![GET /api/devices/{id}/sensors](evidencias/print_get_api-devices-id-sensors.png)
...
![POST /api/devices/{deviceId}/readings](evidencias/print_post_api-devices-devideid-readings.png)
...
![GET /api/devices/{deviceId}/readings](evidencias/print_get_api-devices-deviceid-readings.png)
...
![GET /api/devices/{deviceId}/risk-assessment](evidencias/print_get_api-devices-deviceid-risk-assessment.png)
...
![POST /api/devices/{deviceId}/sync](evidencias/print_post_api-devices-deviceid-sync.png)
...
- Se antes do sync você registrar uma ou mais leituras com `connectionStatusAtCollection: 2`, elas ficam com `isSynced: false`. Nesse caso, o sync retornará a quantidade sincronizada:
...
![GET /api/devices/{deviceId}/sync-logs](evidencias/print_get_api-devices-deviceid-synclogs.png)
...
![POST /api/devices/{deviceId}/alerts](evidencias/print_post_api-devices-deviceid-alerts.png)
...
Para a evidência final, prefira usar `severity` com valores `1`, `2` ou `3`. O arquivo capturado mostra `severity: 0`, que não é ideal para a apresentação da regra de severidade.

### GET `/api/devices/{deviceId}/alerts`

![GET /api/devices/{deviceId}/alerts](evidencias/print_get_api-devices-deviceid-alerts.png)
...
![DELETE /api/devices/{deviceId}/alerts/{alertId}](evidencias/print_delete_api-devices-deviceid-alerts-alertid.png)
...
- Se você executar o mesmo DELETE duas vezes, a segunda chamada tende a retornar `404 Not Found`.
...
![DELETE /api/devices/{id}](evidencias/print_delete_api-devices-id.png)
...
- Se você tentar remover novamente o mesmo device, a resposta será `404 Not Found`.
...
- [x] API executando no terminal.
- [x] Swagger aberto.
- [x] Evidências de POST, GET, PATCH e DELETE.
- [x] Evidências de banco e persistência.
- [x] Evidências de domínio, sensores, leituras, risco, alertas e sincronização.
- [x] Imagem de credenciais redigida.
- [ ] Regenerar, se desejar, a response duplicada de alertas.
- [ ] Regenerar, se desejar, o cadastro manual de alerta com `severity` entre `1` e `3`.
